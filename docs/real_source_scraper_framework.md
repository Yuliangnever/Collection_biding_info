...
旧入口 `https://www.crpsz.com/` 存在证书域名不匹配问题，已从配置中移除。程序会通过华润守正新站和守正招标采购域名继续发现公告。
...
该地址的 `#/purchase?top=0` 是浏览器前端路由，普通 HTTP 请求不会把 `#` 后面的内容发送给服务器。当前服务器返回 `412` JS/WAF 校验页面，需要浏览器自动化、人工 Cookie，或公开接口适配。
...
当前服务器返回 `412` JS/WAF 校验页面，需要浏览器自动化、人工 Cookie，或公开接口适配。
